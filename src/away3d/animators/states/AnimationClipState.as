	...
	public class AnimationClipState extends AnimationStateBase
	{
		...
		override public function update(time:int):void
		{
			if (!_animationClipNode.looping) {
				//trace(time,AnimatorBase(_animator).startTime,_startTime)
				if (time - AnimatorBase(_animator).startTime > _startTime + _animationClipNode.totalDuration)
					time = _startTime + _animationClipNode.totalDuration+AnimatorBase(_animator).startTime;
		...
		protected function updateFrames() : void
		{
			...
			var time : int = _time;
			if (!looping) {
				time -= AnimatorBase(_animator).startTime;
			}
			...
			//trace(_animationClipNode.name,_currentFrame,"99999947444444444444444444444")
		}
		
		private function notifyPlaybackComplete():void
		{
			(_animator as AnimatorBase).dispatchEvent(_animationStatePlaybackComplete ||= new AnimationStateEvent(AnimationStateEvent.PLAYBACK_COMPLETE, _animator, this, _animationClipNode));
