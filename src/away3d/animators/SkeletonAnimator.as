package away3d.animators
{
	...
	public class SkeletonAnimator extends AnimatorBase implements IAnimator
	{
		...
		//存储的skeletonPos
		private var _skeletonPosObj:Object = new Object();
		private function getSkeletonPosByAction(actionName:String,frame:int):Vector.<JointPose>
		{
			if (_skeletonPosObj[actionName]) {
				if (_skeletonPosObj[actionName][frame]) {
					return _skeletonPosObj[actionName][frame];
				}
			}
			return null;
		}
		//=======存储数据
		private var _globalMatricesObj:Object = new Object();
		private function getGlobalMatricesByAction(actionName:String,frame:int):Vector.<Number>
		{
			if (_globalMatricesObj[actionName]) {
				if (_globalMatricesObj[actionName][frame]) {
					return _globalMatricesObj[actionName][frame];
				}
			}
			return null;
		}
		public function get globalMatricesObj():Object 
		{
			return _globalMatricesObj;
		}
		
		public function get skeletonPosObj():Object 
		{
			return _skeletonPosObj;
		}
		//设置数据
		public function setSkeleTonData(arr:Array):void
		{
			_skeletonPosObj = arr[0];
			_globalMatricesObj = arr[1];
		}
		
		public function copySaveData(copyAnimator:SkeletonAnimator ):void
		{
			setFrameAnimation(copyAnimator.frameAnimation);
			if (_frameAnimation) {
				_skeletonPosObj = copyAnimator.skeletonPosObj;
				_globalMatricesObj = copyAnimator.globalMatricesObj;
			}
		}
		
		public function clone():IAnimator
		{
			var animator:SkeletonAnimator = new SkeletonAnimator(_animationSet as SkeletonAnimationSet, _skeleton, _forceCPU);
			animator.copySaveData(this);
			return animator;
		}
		
		public function dispose():void
		{
			AnimationManage.getInstance().deleteAnimation(this);
			stop();
			this._activeSkeletonState = null;
			...
			_skeletonPosObj = null;
			_globalMatricesObj = null;
			//clearSaveDatas();
		}
		 ...
		 */
		public function play(name : String, transition : IAnimationTransition = null, offset : Number = NaN,beginFrame:int = 0) : void
		{
			/*if (_name == name)
				return;*/
			...
			//====动作偏移
			_beginFrame = beginFrame * AnimationClipNodeBase(_activeNode).totalDuration / AnimationClipNodeBase(_activeNode).lastFrame;
		...
		private function updateGlobalProperties() : void
		{
			...
			if (_frameAnimation) {
				var matrices:Vector.<Number> = getGlobalMatricesByAction(_name, (_activeSkeletonState as SkeletonClipState).currentFrame);
				if (matrices) {
					_globalMatrices = matrices;
					return;
				}
				throw new Error("没有初始化骨骼动画")
			}
		...
		{
			if (_frameAnimation) {
				var cunPos:Vector.<JointPose> = getSkeletonPosByAction(_name, (_activeSkeletonState as SkeletonClipState).currentFrame);
				if (cunPos) {
					targetPose.jointPoses = cunPos;
					return;
				}
			}
