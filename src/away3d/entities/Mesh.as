package away3d.entities
{
	...
	public class Mesh extends Entity implements IMaterialOwner, IAsset
	{
		...
		public function deleteMe():void
		{
			...
			var tempM:MaterialBase = material;
			material = null;
			if(tempM){
				if (tempM.owners.length == 0) {
					if (tempM != DefaultMaterialManager.defaultMaterial) {
						tempM.dispose();
					}else {
						tempM.dispose();
						DefaultMaterialManager.defaultMaterial = null;	
					}
				}
			}
			var tempG:Geometry = geometry;
			if (tempG) {
				geometry = null;
				tempG.dispose();
			}
		...
		*/
		...
		//复制 不带动画
		public function cloneNoAimaator() : Mesh
		{
			var clone : Mesh = new Mesh(_geometry, _material);
			clone.transform = transform;
			clone.pivotPoint = pivotPoint;
